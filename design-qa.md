...
- state: dark mobile entry screen, then expanded landscape overlay after a completed OCR scan
...
- Copy and content: permission, privacy, OCR, manual fallback, abnormal-lineup, multi-position, and recommendation labels are concise and specific to the Android workflow.
...
- Overlay bubble expanded in landscape, then hid for capture and returned after OCR.
- OCR detected 4 allies, all 5 supplied enemies, and the banned hero 盾山.
...
- [x] Verify OCR, BAN exclusion, abnormal counts, multi-position weighting, and recommendation output.
